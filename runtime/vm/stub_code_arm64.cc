...
void StubCode::GenerateGuardPageContinuationStub(Assembler* assembler) {
  ...
  // to Dart).
  __ str(R0, Address(SP, -1 * kWordSize, Address::PreIndex));
  intptr_t pc_word_difference = 0;
  for (Register reg = R30;
       reg >= R0;
       reg = static_cast<Register>(static_cast<int>(reg) - 1)) {
    if (reg == SP || reg == FP) continue;
    __ str(reg, Address(SP, -1 * kWordSize, Address::PreIndex));
    pc_word_difference++;
  }

  // Make an artificial exit frame.  We do this because the stack walker
  // assumes the stack always starts with an exit frame.
  __ stp(FP, LR, Address(SP, -2 * kWordSize, Address::PairPreIndex));
  __ mov(FP, SP);
  __ stp(PP, CODE_REG, Address(SP, -2 * kWordSize, Address::PairPreIndex));
  pc_word_difference += 2;

  // Save old C stack pointer.
  __ mov(R0, CSP);
  __ str(R0, Address(SP, -1 * kWordSize, Address::PreIndex));
  ...
  __ add(R0, FP, Operand(pc_word_difference * kWordSize));
  ...
  __ ldp(PP, CODE_REG,  Address(SP, 2 * kWordSize, Address::PairPostIndex));
  __ ldp(FP, LR, Address(SP, 2 * kWordSize, Address::PairPostIndex));
  ...
  for (Register reg = R0;
       ...
       reg = static_cast<Register>(static_cast<int>(reg) + 1)) {
    if (reg == SP || reg == FP) continue;
    __ ldr(reg, Address(SP, 1 * kWordSize, Address::PostIndex));
  }

  // NOTE: Due to us using `ret` or `br` for returning to Dart code, we are
  // forced to clobber one register (either `lr` or `TMP).  As opposed to
  // 32-bit arm, we cannot restore the entire processor state!
  ...
  __ ldr(TMP, Address(SP, 1 * kWordSize, Address::PostIndex));
  __ br(TMP);
