...
void StubCode::GenerateGuardPageContinuationStub(Assembler* assembler) {
  ...
  for (Register reg = R15;
       reg >= RAX;
       reg = static_cast<Register>(static_cast<int>(reg) - 1)) {
    if (reg == RBP || reg == RSP) continue;
    __ pushq(reg);
    pc_word_difference++;
  }
  ...
  ASSERT(pc_word_difference * 8 == 128);
  __ leaq(RDI, Address(RBP, pc_word_difference * 8));
  __ leaq(RSI, Address(RBP, 8));
  __ movq(RDX, RBP);

  __ PushRegisters(0, CallingConventions::kVolatileXmmRegisters);
  ...
  __ call(Address(THR, Thread::interruption_continuation_fun_offset()));

  // Undo padding.
  __ movq(RSP, RBX);

  __ PopRegisters(0, CallingConventions::kVolatileXmmRegisters);
