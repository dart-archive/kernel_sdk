...
void Library::WriteTo(Writer* writer) {
  ...
  writer->WriteUInt32(classes_.length());
...
void Reference::WriteMemberTo(Writer* writer, Member* member) {
  ...
  if (node->IsLibrary()) {
    ...
    if (member->IsField()) {
      ...
      writer->WriteUInt32(helper->libraries().Lookup(library));
      writer->WriteUInt32(helper->fields().Lookup(field));
    } else {
      Procedure* procedure = Procedure::Cast(member);
      writer->WriteTag(kLibraryProcedureReference);
      writer->WriteUInt32(helper->libraries().Lookup(library));
      writer->WriteUInt32(helper->procedures().Lookup(procedure));
    }
  } else {
    ...
    if (member->IsField()) {
      ...
      Reference::WriteClassTo(writer, klass);
      writer->WriteUInt32(helper->constructors().Lookup(constructor));
    } else {
      Procedure* procedure = Procedure::Cast(member);
...
void Reference::WriteClassTo(Writer* writer, Class* klass) {
  ...
  writer->WriteUInt32(writer->helper()->libraries().Lookup(klass->parent()));
  writer->WriteUInt32(writer->helper()->classes().Lookup(klass));
}
...
VariableGet* VariableGet::ReadFrom(Reader* reader) {
  ...
  get->variable_ = reader->helper()->variables().Lookup(reader->ReadUInt32());
  return get;
}
...
void VariableGet::WriteTo(Writer* writer) {
  ...
  writer->WriteUInt32(writer->helper()->variables().Lookup(variable_));
}

VariableSet* VariableSet::ReadFrom(Reader* reader) {
  TRACE_READ_OFFSET();
  VariableSet* set = new VariableSet();
  set->variable_ = reader->helper()->variables().Lookup(reader->ReadUInt32());
...
IntLiteral* IntLiteral::ReadFrom(Reader* reader) {
  ...
  literal->value_ = negative ? -static_cast<int64_t>(reader->ReadUInt32()) : reader->ReadUInt32();
  return literal;
}
...
void IntLiteral::WriteTo(Writer* writer) {
  ...
  writer->WriteUInt32(static_cast<uint32_t>(value_ < 0 ? -value_ : value_));
}
...
BreakStatement* BreakStatement::ReadFrom(Reader* reader) {
  ...
  stmt->target_ = reader->helper()->lables().Lookup(reader->ReadUInt32());
  return stmt;
}
...
void BreakStatement::WriteTo(Writer* writer) {
  ...
  writer->WriteUInt32(writer->helper()->lables().Lookup(target_));
}
...
ContinueSwitchStatement* ContinueSwitchStatement::ReadFrom(Reader* reader) {
  ...
  stmt->target_ = reader->helper()->switch_cases().Lookup(reader->ReadUInt32());
  return stmt;
}
...
void ContinueSwitchStatement::WriteTo(Writer* writer) {
  ...
  writer->WriteUInt32(writer->helper()->switch_cases().Lookup(target_));
}
...
void Name::WriteTo(Writer* writer) {
  ...
  writer->WriteUInt32(writer->helper()->libraries().Lookup(library_));
}
...
void FunctionType::WriteTo(Writer* writer) {
  ...
  type_parameters_.WriteTo(writer);
  writer->WriteUInt32(required_parameter_count_);
...
TypeParameterType* TypeParameterType::ReadFrom(Reader* reader) {
  ...
  type->parameter_ = reader->helper()->type_parameters().Lookup(reader->ReadUInt32());
  return type;
}
...
void TypeParameterType::WriteTo(Writer* writer) {
  ...
  writer->WriteUInt32(writer->helper()->type_parameters().Lookup(parameter_));
}
...
void FunctionNode::WriteTo(Writer* writer) {
  ...
  type_parameters().WriteTo(writer);
  writer->WriteUInt32(required_parameter_count());
